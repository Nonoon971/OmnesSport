...
echo "<style>#reserve{background-color : blue ;}
  ...
  //  $coachName= isset($_POST["nom"])? $_POST["nom"] : "";
    ...
    if($db_found){
    	$sql="SELECT Heure,jour FROM rdv WHERE Sport='$Sport'";
        ...
        if($i==5){$marque=50;}
				...
				for($y=0;$y<count($DayALL);$y++){
					...
					if($test==1){
						...
						//	echo "<td id='". (string)$marque ."'><a href='reservation.php?Jour=".$HorraireALL[$i]."&Heure=".$DayALL[$y]."&Sport=".$Sport."'>". $HorraireALL[$i] ."</a></td>";
								echo "<td id='". (string)$marque ."'><a href='paiement_boxe.php?Jour=".$HorraireALL[$i]."&Heure=".$DayALL[$y]."&Sport=".$Sport."'>". $HorraireALL[$i] ."</a></td>";
							$marque=$marque-$y;
					}
				}
				echo  "</tr>";
